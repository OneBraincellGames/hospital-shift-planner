...
export function PlannerConfigForm({ initialConfig }: Props) {
  ...
  function set(field: FieldKey, raw: string) {
    const n = parseInt(raw, 10);
    setValues((v) => ({ ...v, [field]: isNaN(n) ? 0 : Math.max(0, Math.min(23, n)) }));
    setSaved(false);
  }

  function setUnbounded(field: FieldKey, raw: string) {
    const n = parseInt(raw, 10);
  ...
  const shiftRows: { key: keyof typeof t.shifts; startField: FieldKey; endField: FieldKey }[] = [
    { key: "EARLY", startField: "earlyShiftStart", endField: "earlyShiftEnd" },
    { key: "LATE",  startField: "lateShiftStart",  endField: "lateShiftEnd"  },
    { key: "NIGHT", startField: "nightShiftStart", endField: "nightShiftEnd" },
    { key: "DAY",   startField: "dayShiftStart",   endField: "dayShiftEnd"   },
  ];
  ...
  return (
    <div className="space-y-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => setUnbounded("minRestHours", e.target.value)}
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => setUnbounded("maxConsecutiveDays", e.target.value)}
                className="w-24 border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <p className="text-xs text-gray-400 mb-5">{t.plannerConfig.shiftTimesHint}</p>

        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-500 border-b border-gray-100">
                <th className="pb-2 font-medium w-20">Schicht</th>
                <th className="pb-2 font-medium w-32">{t.plannerConfig.startLabel}</th>
                <th className="pb-2 font-medium w-32">{t.plannerConfig.endLabel}</th>
                <th className="pb-2 font-medium">{t.plannerConfig.durationLabel}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {shiftRows.map(({ key, startField, endField }) => {
